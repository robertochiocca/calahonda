...
def generate_pdf_report(
    ...
    plt.rcParams.update({
        ...
        "savefig.facecolor": _SURFACE, "text.color": _INK,
        "axes.edgecolor": "#2A2F3E", "axes.labelcolor": _MUTED,
    ...
    with PdfPages(output) as pdf:
        ...
        fig.text(0.07, 0.925, f"Gerado em {date.today():%d/%m/%Y} · "
                 f"carteira de R$ {portfolio_value:,.0f} · VaR {confidence:.0%} · "
                 f"{horizon_days} dias úteis", fontsize=9, color=_MUTED)
        ...
        lines = [
            ...
            ("Backtest de VaR (Kupiec)",
             ...
             f"{bt.kupiec_pvalue:.3f} · {'APROVADO' if bt.approved else 'REPROVADO'}"),
        ]
        y = 0.86
        fig.text(0.07, y + 0.025, "Métricas da carteira", fontsize=12,
        ...
        def _table(ax, df, title_text):
            ...
            ax.set_title(title_text, fontsize=11, fontweight="bold",
                         color=_INK, loc="left", pad=14)
            data = df.round(2).astype(str)
            # índice como primeira coluna, para nada vazar da página
            cell_text = [[str(i)] + row for i, row in
                         zip(data.index, data.values.tolist())]
        ...
        fig.text(0.07, 0.05, "Documento educacional gerado por código aberto — "
                 "não é recomendação de investimento.", fontsize=8, color=_MUTED)
        pdf.savefig(fig)
        plt.close(fig)
        ...
        ax.hist(terminal, bins=60, color=_LIME, alpha=0.85,
        ...
        ax.text(-var_mc * 100, ax.get_ylim()[1] * 0.95,
                f" VaR {confidence:.0%} = {var_mc:.1%}", color=_AMBER, fontsize=9)
        ax.text(-cvar_mc * 100, ax.get_ylim()[1] * 0.87,
                f" CVaR = {cvar_mc:.1%}", color=_RED, fontsize=9, ha="right")
        ax.set_title(f"Monte Carlo — P&L simulado em {horizon_days} dias "
                     "(10.000 cenários)", loc="left", fontsize=12, fontweight="bold")
